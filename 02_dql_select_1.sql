...
SELECT
    menu_name as 메뉴명,
    menu_price as "메뉴 가격", # 쌍 따옴표를 이용해서 띄어쓰기 된 컬럼 묶기
    menu_price * 0.1 부가세,
...
select
    concat(menu_name,' ',menu_price, '원') as "메뉴 가격(원)"
from
    tbl_menu;
...
# - 특정 컬럼을 기준으로 삼어서 오름차순(ASC), 내림차순(DESC) 지정
...
# select절에 작성되지 않는 컬럼도 정렬 기준이 될 수 있다
select
    menu_name
from
    tbl_menu
...
# 반대 -> 10,000원 미만 또는 20,000 초과
# 10,000원 이상, 20,000 이하
select
    menu_name,
    menu_price,
    category_code
...
# 카테고리 테이블에서 ref_category_code가 null인 행 조화
select
    *
from
    tbl_category
where
    ...
    # ref_category_code 칼럼이 비어있으면 True
...
# offset = 건너뛰기 - 없다고 생각하고 ex) n == 5면 5칸 건너뛰고 6부터
